...
export class LocationService {
    ...
    // 当前链接元素
    private readonly anchorElement = document.createElement('a');
    ...
    handleAnchorClickEvent(anchorElement: HTMLAnchorElement, mouseClicks = 0, ctrlPressed = false, metaPressed = false): boolean {
        ...
        this.anchorElement.href = anchorHref;

        // 链接是外部链接，或者链接中存在扩展名时，让浏览器处理链接的点击事件
        if ((!isInPageAnchor && anchorElement.href !== this.anchorElement.href) || !/\/[^/.]*$/.test(anchorElement.pathname)) {
            return true;
        }
